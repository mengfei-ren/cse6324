...
public class Statement {
	...
	public static Statement getRandomizedStatement(Method method, ArrayList<ClassGenerator> classList) {
		...
		case 0: // Assignment statement
			...
			// FIXME: if there is no local variable available to assign, it will simply use
			// print statement.
		...
		case 2: // method calls: restrict it to MAX_ALLOWED_METH_CALL
			...
			if (methCalledCounterValue < method.getMaxAllowedMethodCalls()) {
				if (ProgGenUtil.coinFlip()) {
					...
					String methodCall = ProgGenUtil.getMethodCallForReturnType(method, classList,
							new Type(selectedPrimitive, ""), lhs);
