...
public class ForLoop extends Statement {
	...
	public ForLoop(Method method, ArrayList<ClassGenerator> classList) {
		...
		if (option == 0) {
			...
			// We don't want method calls in the for loop, allow print statements that has
			// 'method' keyword in it
			while (stmt != null && stmt.toString().contains("method")
					&& !stmt.toString().contains("System.out.println")) {
				stmt = Statement.getRandomizedStatement(method, classList);
			}
		}
	}
