...
/**
 * Creates Single entry classes in a tree structure. For X number of generated
 * classes, it will create Y number of SingleEntry class such that Y = X/150.
 * That is, each of Y classes will call 150 SingleEntry methods of generated
 * classes; this is the leaf level.
 * 
...
public class TreeOfSingleEntryGenerator {
	...
	private void createClass(int level, int target) {
		if (level == 0) {
			...
			/*
			 * create 'loop' number of classes where each class calls 150 or less
			 * T.SingleEntry() methods in total 'target' number of T.SingleEntry() methods
			 * name these classes TStart_Llevel_X
			 */
			...
			for (int i = 0; i < loop; i++) {
				...
				try {
					File file = new File(DirPath + "TestPrograms" + File.separator + "com" + File.separator
							+ "accenture" + File.separator + "lab" + File.separator + "carfast" + File.separator
							+ "test" + File.separator + "TStart_L" + level + "_" + i + ".java");
					...
					for (int j = 0; j < methCallLimit && count < target; j++, count++) {
						...
						output.append(ConfigurationXMLParser.getProperty("classNamePrefix") + count + ".singleEntry("
								+ argument + ");\n");
					}
			...
			if (level == LEVEL) {
				...
				try {
					File file = new File(DirPath + "TestPrograms" + File.separator + "com" + File.separator
							+ "accenture" + File.separator + "lab" + File.separator + "carfast" + File.separator
							+ "test" + File.separator + ConfigurationXMLParser.getProperty("classNamePrefix") + "Start"
							+ ".java");
				...
				for (int i = 0; i < loop; i++) {
					/*
					 * create 'loop' number of classes,where each calls 150 or less
					 * TStart_L(prevlevel)_X.entryMethod() methods; in total 'target' number of
					 * calls.
					 * 
